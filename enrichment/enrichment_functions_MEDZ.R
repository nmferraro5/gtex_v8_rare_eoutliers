...
filter.outliers <- function(stats_file, threshold, topn = FALSE) {
    ...
    if (method == 'MEDZ') {
        ...
        #outliers = outliers %>% top_n(-1000, Rank) # Added just to compare PEER - delete later??
    ...
    if (topn) {
        outliers = outliers %>% top_n(-abs(threshold), Rank) # negative first argument to get smallest n
    } else {
        outliers = outliers %>% filter(Rank <= threshold)
    }
...
get.features <- function(individual, feature_dir, feature_suffix, outliers) {
     # outliers = outliers %>% mutate(Method = 'MEDZ') %>%
     #     filter(Ind == individual) %>%
     #     mutate(Y = ifelse(Y == 'outlier', 1, 0), 
     #            Gene = sub('\\.[0-9]+','', Gene)) 
    outliers = outliers %>% filter(Ind == individual)
    if ('Specific.Group' %in% colnames(outliers)) {
        outliers = select(outliers, Gene, Ind, Y, Method, Specific.Group, Maf, Type)
    } else {
        outliers = select(outliers, Gene, Ind, OutlierValue, Y, Method, Maf, Type)
    }
    ...
    features$n_variants = as.integer(as.character(features$n_variants))
...
compute.relative.risk <- function(maf, vartype, outliers, prefix, suffix, scale, counts = TRUE) {
  ...
  if ('Specific.Group' %in% colnames(outliers_with_feats)) {
    ...
    start_ind = 8 # previously 7
  }
  features = colnames(outliers_with_feats)[start_ind:length(colnames(outliers_with_feats))]  
  print(outliers_with_feats[1,])
  print(table(outliers_with_feats$Y))
...
compute.continuous.risk <- function(maf, vartype, outliers, prefix, suffix, scale, counts = TRUE) {
  ## Get merged outliers with features
  method = outliers$Method[1]
  cat(method, maf, vartype, '\n')
  outliers_with_feats = get.all.features(outliers, maf, vartype, prefix, suffix)
  ...
  features = colnames(outliers_with_feats)[start_ind:length(colnames(outliers_with_feats))]  
  estims = data.frame(Beta = numeric(), SE = numeric(), Pval = numeric(), Feature = character())
  if (method == 'TotalExpression') {
    outliers_with_feats$OutlierValue = sapply(outliers_with_feats$OutlierValue, function(x) 2*pnorm(-abs(x)))
  }
  featurevar = var(outliers_with_feats$any_variant, na.rm = TRUE)
  if (!is.na(featurevar) && featurevar > 0) {
    if (method == 'TotalExpression') {
      #lmod = summary(lm(-log10(OutlierValue) ~ any_variant, data = outliers_with_feats))$coefficients
      lmod = summary(glm(any_variant ~ scale(-log10(OutlierValue)), data = outliers_with_feats, family=binomial))$coefficients
      #lmod = summary(glm(-log10(OutlierValue) ~ scale(n_variants), data = outliers_with_feats))$coefficients
    } else {
      outliers_with_feats = filter(outliers_with_feats, !is.na(OutlierValue))
      #lmod = summary(lm(-log10(OutlierValue+0.00000001) ~ any_variant, data = outliers_with_feats))$coefficients
      lmod = summary(glm(any_variant ~ scale(-log10(OutlierValue+0.00000001)), data = outliers_with_feats, family=binomial))$coefficients
      #lmod = summary(glm(-log10(OutlierValue+0.00000001) ~ scale(n_variants), data = outliers_with_feats))$coefficients
    }
    estims = rbind(estims, data.frame(Beta = lmod[2,1],
                                      SE = lmod[2,2],
                                      Pval = lmod[2, 4],
                                      Feature = 'any_variant'))
  } else {
...
get.all.enrich <- function(outliers, feature_dir, counts = FALSE, scale = TRUE, suffix = NULL) {
    ...
    #mafs = c('MAF0-1','MAF0-0.1','MAF0.1-1')
    #mafs = c('MAF0-1', 'MAF1-5', 'MAF5-10', 'MAF10-25')
    #mafs = c('MAFnovel', 'MAFac1', 'MAFac2', 'MAFgnomad0-1only', 'MAFgnomad1-5')
    #mafs = c('MAFnovel', 'MAFgnomad0-1only', 'MAFgnomad1-5')
    #mafs = c('MAF0-single', 'MAFsingle-double', 'MAF0-1', 'MAF1-5')
    mafs = c('MAF0-single', 'MAF0-1', 'MAF1-5')
    if (!counts) {
#        mafs = 'MAF0-1'
        mafs = mafs[!(mafs == 'counts')] # removes counts dir, if there
    }
    vartypes = list.dirs(paste0(feature_dir, '/', mafs), full.names = FALSE, recursive = FALSE)
    ...
    vartypes = c('HallLabSV') # adding to combine enrichments for SNPs+indels
    #vartypes = c('SNPs+indels')
