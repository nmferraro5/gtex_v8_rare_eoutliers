...
get.features <- function(individual, feature_dir, feature_suffix, outliers) {
    ...
    indiv_file_down = paste0(feature_dir, individual, '_counts_downstream_bygene.txt')
    features = fread(indiv_file, sep = '\t', header = TRUE, fill=T) %>%
        mutate(Gene = sub('\\.[0-9]+','', gene_id)) %>% # same as above
        select(-gene_id) %>% filter(Gene %in% genes)
...
compute.relative.risk <- function(maf, vartype, outliers, prefix, suffix, scale, counts = TRUE) {
  ...
  cat(method, maf, vartype, '\n')
  outliers_with_feats = get.all.features(outliers, maf, vartype, prefix, suffix)
  if (length(outliers_with_feats) == 0) {
      return(data.frame(Riskratio = NA, Lower = NA, Upper = NA, Pval = NA, Feature = NA, Method = method, Type = vartype, Maf = maf))
  }
  ## if counts features, then create and 'any' feature and change the name of the exisiting feature
  ## (so it doesn't clash with the n_variants of the full feature set)
  ## if the full feature set, zero-impute NAs for CADD TFBS features
  ...
  for (feature in features) {
    ...
    tryCatch({
      if (!is.na(featurevar) && featurevar > 0) {
        ...
        #   counttable = rbind(table(controls[,feature])+1,
        #                      table(outliers[,feature])+1)
        # } else {
        #   counttable = rbind(c(table(controls[,feature])+1,1),
        #                      table(outliers[,feature])+1)
        # }
        if (length(unique(outliers[,feature])) > 1) {
          ...
          counttable = rbind(c(table(controls[,feature]),1),
                             table(outliers[,feature]))
        }
        #counttable = rbind(table(filter(outliers_with_feats,Y==0)[,feature]),
        #                   table(filter(outliers_with_feats,Y==1)[,feature]))
...
get.all.enrich <- function(outliers, feature_dir, counts = FALSE, scale = TRUE, suffix = NULL) {
    if (is.null(suffix)) {
        if (counts) {
            ...
            suffix = '_counts_upstream_bygene.txt'
            scale = FALSE
        } else {
    ...
    #mafs = c('MAFnovel', 'MAFac1', 'MAFac2', 'MAFgnomad0-1', 'MAFgnomad1-5')
    mafs = c('MAFnovel', 'MAF0-single')
    #mafs = c('MAFnovel', 'MAFgnomad0-1only', 'MAF0-double', 'MAF0-1')
    if (!counts) {
#        mafs = 'MAF0-1'
    ...
    #vartypes = vartypes[which(vartypes != 'HallLabSV')] # Remove SVs for gnomad AF
